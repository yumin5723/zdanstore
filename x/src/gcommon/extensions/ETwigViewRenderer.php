...
class ETwigViewRenderer extends CApplicationComponent implements IViewRenderer
{
     ...
     */
    public $twigPathAlias = 'gcommon.lib.Twig';
    /**
    ...
    function init()
    {
        ...
        /** @var $theme CTheme */
        if ($this->loader_type == "file") {
            $theme = $app->getTheme();
            if ($theme === null) {
                $this->_basePath = $app->getViewPath();
            } else {
            ...
            // add module view path to twig loader
            if (($module=Yii::app()->getController()->getModule()) !== null) {
                $loader->addPath($module->getViewPath(), $module->getId());
            }
        } elseif ($this->loader_type == "string") {
    ...
    /**
     * render partical file
     */
    public function renderPartial($views,$data,$context=null){
        if(($viewFile=$this->resolveViewFile($views,$this->_basePath)) !== false){
            return $this->renderFile($context,$viewFile,$data,true);
        }else{
            throw new CException(Yii::t('Yii','{controller} cannot find the request view "{view}".',array('controller'=>get_class($this),'{view}'=>$view)));
        }
    }
    public function resolveViewFile($viewName,$viewPath=null,$basePath=null,$moduleViewPath=null)
    {
        if(empty($viewName))
            return false;
        if ($basePath===null) {
        ...
        if ($viewPath===null) {
            $viewPath=Yii::app()->getViewPath().DIRECTORY_SEPARATOR.'noController';
        }
    ...
    {
        require_once Yii::getPathOfAlias("gcommon.extensions").'/Twig1378Ext.php';
 ...
 * @return [type]         [description]
 */
function getCmsBlock($block_id){
    Yii::import('gcommon.cms.models.Block');
    $html = Block::model()->findByPk($block_id);
    if(empty($html)){
        return "";
    }
    return $html->html;
