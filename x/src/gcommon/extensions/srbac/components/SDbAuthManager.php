...
class SDbAuthManager extends CDbAuthManager {
    ...
    /**
     * bizRule for assignment with category id
     *
     * @param $params array have cid as request category id
     * @param $data allowed category ids seperate by "|"
     *
     * @return
     */
    public function withCategory($params, $data) {
        if (empty($params['cid'])) {
            return false;
        }
        $allow_cids = explode('|', $data);
        return in_array($params['cid'], $allow_cids);
    }
