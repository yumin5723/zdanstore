 ...
 */
class ProductTerm extends CActiveRecord
{
    /**
     ...
     * @return
     */
    public function getAllTermsRefObject($product_id) {
        return array_map(function($term){return $term->term_id;},
            $this->findAllByAttributes(array('product_id'=>intval($product_id))));
    }
    /**
     ...
     * @return boolean
     */
    public function updateProductTerm($product_id,$terms){

        
        // get current dependence
        $current = $this->getTermsByProduct($product_id);
        ...
        // save to db
        if (!empty($to_del)) {
            $this->removeTerms($product_id,$to_del);
        }
        if (!empty($to_insert)) {
            $this->addTerms($product_id,$to_insert);
        }
        return true;
    }
    /**
     ...
     * @return
     */
    protected function getTermsByProduct($product_id) {
