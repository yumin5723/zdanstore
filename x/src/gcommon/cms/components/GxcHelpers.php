...
class GxcHelpers {
    ...
    **/
    public static function renderTextBoxResourcePath($data){
        return '<input type="text" class="pathResource" onClick="return selectAllText(this);" value="'.Yii::app()->params['resource_url']."/".$data->resource_path.'" />';
    }
    ...
    **/
    public static function renderLinkPreviewResource($data){
        switch($data->resource_type){
            case 'image':
                return '<a href="'.$data->getFullPath().'" rel="prettyPhoto" title="'.$data->resource_name.'">'.Yii::t('cms','View').'</a>';
                break;
            case 'video':
                return '<a href="/js/jwplayer/player.swf?width=470&amp;height=320&flashvars=file='.$data->getFullPath().'" title="'.$data->resource_name.'" rel="prettyPhoto">'.Yii::t('cms','View').'</a>';
                break;
            default:
                return '<a href="'.$data->getFullPath().'">'.Yii::t('cms','View').'</a>';
                break;
