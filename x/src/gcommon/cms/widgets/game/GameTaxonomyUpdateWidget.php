 ...
 *
 */
class GameTaxonomyUpdateWidget extends CWidget
{
    ...
    {       
        ...
        $model = GameTaxonomy::model()->findByPk((int)$id);
        ...
        $list_terms=GameTerm::model()->findAll(
                 array(
                     'select'=>'*',
        ...
        if(isset($_POST['GameTaxonomy']))
        {
                $model->attributes=$_POST['GameTaxonomy'];                        
                if($model->save()){                            
                    Yii::app()->user->setFlash('success',Yii::t('cms','Update Taxonomy Successfully!'));                                                           
                }
        }
        $this->render('cmswidgets.views.gametaxonomy.game_taxonomy_form_widget',array('model'=>$model,'lang_exclude'=>$lang_exclude,'versions'=>$versions,'type'=>$type,
