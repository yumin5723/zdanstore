...
class MessageController extends GController {
    ...
    public function actionReply(){
        ...
        if(isset($_POST['Message'])){
            ...
            $count = $model->updateByPk($id,array("reply"=>$_POST['Message']['reply'],'isreply'=>1,"modified"=>$date));
            $data = $model->findByPk($id);
            if ($count) {
                Yii::app()->user->setFlash('success', Yii::t('cms', 'Reply user answer is successfully!'));
                return $this->render('create',array("model"=>$data));
            }
        }
