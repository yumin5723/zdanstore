...
class ShippingController extends GController {
    ...
    public function loadModel($id) {
        $model = Shipping::model()->findByPk((int)$id);
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Shipping'])) {
            $model->setAttributes($_POST['Shipping']);
            // validate user input password
            if ($model->validate()) {
                if ($model->save()) {
                    Yii::app()->user->setFlash('success', Yii::t('cms', 'Create new Shipping Successfully!'));
                    $this->redirect('admin');
                }
            }
        }
    ...
    public function actionUpdate() {
        ...
        if ($id !== 0) {
            ...
            // collect user input data
            if (isset($_POST['Shipping'])) {
                if ($model->updateAttrs($_POST['Shipping'])) {
