 ...
 function buildResourceList(id,link,key,file_type){
        ...
        var link_resource_icon='/images/content_icons/'+file_type+'.png';
        if(file_type=='image'){
            ...
            '<li id="list_id_resource_upload_'+key+'_'+next_resource_count+'" align="left" style="background: transparent; float:left" >'+
                ...
                '<div class="item_div_wrapper">'+                                                             
                    ...
                    '<a href="/resource/update/'+id+'" target="_blank"  name="editItem"><img alt="<?php echo  Yii::t('cms','修改'); ?>" src="/images/edit.png" /></a>&nbsp;'+                                                                     
                    '<a href="javascript:void(0)" onclick="fnRemoveResourceItem(\'list_id_resource_upload_'+key+'_'+next_resource_count+'\',\''+key+'\');" name="removeItem"><img alt="<?php echo  Yii::t('cms','删除'); ?>" src="/images/disabled.png" /></a>'+  
                    '</div>'+                                
                '</div>'+                                                                                                            
...
function doUploadResource(type){
    ...
    if(next_resource_count<=current_resource_max){
        $.prettyPhoto.open('/cms/resource/createframe?content_type=<?php echo $type;?>&parent_call=true&type='+type+'&iframe=true&height=400','<?php echo  Yii::t('cms','上传资源');?>','');
    } else {
        alert('<?php echo  Yii::t('cms','上传文件过大!'); ?>');
        return false;
    }
                ...
                <?php foreach ($content_resources as $key=>$value) : ?>
                    <div class="tab-content" id="<?php echo $key; ?>_box">
                        ...
                        <div class="button_add" style="text-align:right; padding-right:10px; padding-top:10px; padding-bottom:10px">
                            <input type="button" onClick="return doUploadResource('<?php echo $key; ?>');" class="bebutton active" name="<?php echo $key; ?>_button" id="<?php echo $key; ?>_button" value="<?php echo  Yii::t('cms','添加'); ?>"/>
                        </div>
                    </div> 
                <?php endforeach; ?>
            <?php endif; ?>
